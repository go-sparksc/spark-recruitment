...
- **The gate, in order:**
  ...
  4. Download all four artifacts and open the CSVs in Excel. **Re-scoped after slice 4, because the original step was not testable:** none of the three CSVs carries free text — they are identity, status, scores, decisions and emails — so there are no essays in them to check, and response columns are deliberately not being added. What to check instead is the case that is live. Confirm the columns line up on a row whose **name contains a comma** (rename an applicant to `Fang, Cecilia` on `/results/[applicantId]` if the fixture has none, then re-download), and confirm a **formula-leading cell** renders as decision 90 says it will — Excel evaluating `=…` is the documented consequence, not a defect. The line-break branch of the quoter stays covered by `lib/export-csv.test.ts` rather than by the gate, since no file can reach it today.
  5. Run the check. Then the stronger form BUILD_PLAN actually asks for: **drop and re-migrate the whole database** rather than deleting one instance — `prisma migrate reset` with seeding suppressed (confirm the Prisma 7 flag; `migrate dev` cannot run here per CLAUDE.md), then `migrate deploy`, then restore from the downloaded file alone. Deleting one instance proves the cascade; dropping the database proves the export depends on nothing outside it.
