...
6. **Load evenness is bounded on both sides, which is an FR-7 edit rather than a test that outruns the spec.** §7.2 states only the ceiling, and a ceiling alone permits 28 reviewers at 15 and two at 4 and 0 across the 428 assigned slots — satisfying the written rule while defeating "as even as possible". So FR-7 gains a floor. The wording that goes in, in full, since the exemption has to be a computed condition and not a category:

   > **The bound is two-sided.** No reviewer exceeds `ceil(total_slots / reviewer_count)`, and no reviewer falls below `floor(assigned_slots / reviewer_count)`. At 150 applicants and 30 reviewers that is a floor of 14 against a ceiling of 15, so every reviewer carries 14 or 15. A ceiling alone is not evenness: 28 reviewers at 15 and two at 4 and 0 also sums to 428 and also satisfies it.
   >
   ...
   An earlier draft of this exemption said a reviewer was exempt when "already assigned to every applicant eligible for them". That is vacuous: in a complete plan every applicant is already at `target` or `target − 1`, so no applicant has room for anyone, so every reviewer qualifies. The condition has to be about whether a **swap** could have evened the load, not about whether a free slot happens to exist.
...
**The floor is asserted because FR-7 gains it, not because the test wants it.** Order matters here: the PRD edit in decision 6 lands before this slice, so the assertion is checking a stated rule rather than outrunning one. Worked through the cases, the floor is `14` at 30 reviewers, `13` at 31, `9` at 3 reviewers and 10 applicants, and `1` at the 2×2 case — all satisfiable, and all satisfied by the same lowest-load-first fill that the ceiling already drives.

**Where the invariant helper does its work.** For any reviewer below the floor it runs the swap search from decision 6 — over applicants × reviewers, trivial at this scale — and fails if it finds an improving swap the one-Sparklet rule did not block. Of the six cases only relaxed 30/15/150 ever puts a reviewer below the floor, so the other five assert `load ≥ floor` unconditionally and never reach the search. On 30/8/150 in particular the exemption is not merely unfired but unreachable: 428 = 30 × 14 + 8 places everyone at 14 or 15.

**The same search is the generator's repair step**, which is why it is worth writing once and using twice: the triple that proves a violation is exactly the swap that fixes it, so the fill loop can run it to a fixed point rather than carrying a separate rebalancing pass that could disagree with the assertion.
...
- **FR-7** — three edits:
  ...
  - Add the evenness **floor** beside the ceiling, in the wording quoted in full under decision 6: the plain-language rule first so §7.2 stays readable, the swap condition after it as the precise form, and the note that both bounds are evaluated at generation and not maintained afterward. With the reason for the floor, since the gap is not obvious — the ceiling alone admits 28 reviewers at 15 and two at 4 and 0.
