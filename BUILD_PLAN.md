...
- **9.5 — admin surfaces.** **Decision 110** (an included RESPONSE column arrives Reviewer-visible), decision 118, the moved-on filter on both results pages, the mapping screen's two cosmetic tweaks, a real back button, the ethnicity truncation, the two missing `Applicant N` handles below, and **decision 116's AI-flag tooltip** — the cell keeps its bare count and the tooltip gains "N of M completed reviews", per the amendment in 116. Four of those touch `results-table.tsx`, which is why they ship together. Note that 110 is the one *behavioural* change in an otherwise presentational slice — it is here because it lives in the same two mapping actions as the cosmetic tweaks, and it needs its own tests where the rest need looking at.

  **The `Applicant N` handle is missing on two more surfaces**, found by auditing after slice 9.2 hit it on the first-round list. `first-round-results/results-table.tsx` is the one that matters: FR-15's ranked list renders the name alone beside the checkboxes that advance or reject, so two applicants called "Diego Hoffmann" are one indistinguishable pair away from the wrong person being advanced — worse than the list 9.2 fixed, where the cost was opening the wrong profile. Its confirm panel names unvoted rejects by `displayName` alone and can print the same name twice. The first-round applicant profile is the other, and is cosmetic by comparison. FR-12/13's reconciliation picker uses `name — email` instead, which is better where an email exists and degenerates where FR-3 allowed a blank one; recorded, not changed.
