...
- **`npm run verify` does not build, and a `"use server"` module may only export async functions.** Exporting a `const` from an actions file is a *build* error, not a typecheck error, so typecheck, lint and every test go green while `next build` fails — the same shape as the dev-server trap below, one layer earlier. Types and interfaces are fine (they are erased); anything with a runtime value is not. Run `npm run build` once before calling a slice done if it touched a `"use server"` file. Hit in Phase 9.4 on a `ROUND_PREREQUISITES` map that only its own module needed.
