...
**Owner:** Kai Lincoln
**Status:** v1.29, Phases 0-9 complete, decisions recorded through 121 — every slice shipped and gated, and the reconciliation pass in `plans/prd-reconciliation.md` applied. Phase 8's succession gate, a board member running a full mock cycle from `ADMIN_GUIDE.md` alone, is complete and found nothing pressing (`plans/phase-8.md`). Spark's first real cycle is **in progress**, and was stopped at FR-12's upload step until decision 120 shipped on 2026-09-17. The freeze ahead of that cycle has three owner-approved exceptions: decision 119's class standing (`plans/decision-119.md`), decision 120's interview transcript and per-interviewer notes (`plans/decision-120.md`), and decision 121's removal of the written round from the second-round profile. **120 was written as provisional** — built against the cycle's two real sheets, tested on a throwaway instance, adopted on the owner's call after that test. Every other refinement still waits until after the cycle
